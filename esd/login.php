...
if(isset($_GET['act'])){
	...
	// end AD checking
		...
		if ($authUser == true) { // if correct
		    ...
		    $qry=sqlsrv_fetch_array(sqlsrv_query($conn,"SELECT * FROM users WHERE domain='".$_POST['logname']."'"));
			if($qry['id']){
				session_start();
				$_SESSION['esd_username']=$_POST['logname'];
				header("location:index.php");
			}
		    else{
		    	$ermsg="<font color='red' size='+1'>Your account has no access to this document!<br><br></font>";
		    }
		}
	...
	<form class="login-form" action="login.php?act=login" method="post">
		...
		<div class="form-group">
			...
			<div class="input-icon">
				...
				<input class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder="Domain name" name="logname"/>
			</div>
		</div>
		<div class="form-group">
