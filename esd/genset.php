...
if(isset($_GET['delete'])){
	$delete=sqlsrv_query($conn,"delete from genset_utilization where id='".$_GET['id']."'");
	$delete2=sqlsrv_query($conn,"delete from genset_utilizationFlatData where downtimeId='".$_GET['id']."'");
...
if(isset($_GET['act'])){
	if($_GET['act']=='submitdowntime'){
		$insert="insert into genset_utilization (dateStart,dateEnd,remarks,addedBy,addedDate,unitId,fuel,kwh,runStart,runStop)
			VALUES('".$_POST['startd']."','".$_POST['endd']."','".$_POST['remarks']."','".$_SESSION['esd_username']."','".date('Y-m-d h:i:s')."','".$_POST['unit']."','".$_POST['fuel']."','".$_POST['kwh']."','".$_POST['startreading']."','".$_POST['stopreading']."'); SELECT SCOPE_IDENTITY()";
		...
		foreach ($arrd as $key => $value){
			...
			$ins=sqlsrv_query($conn,"insert into genset_utilizationflatdata (date,mins,unitId,downtimeId,remarks,fuel,kwh,runStart,runStop)
				VALUES('".$value."','".$arr[$key]."','".$_POST['unit']."','".$lastins."','".$_POST['remarks']."','".$_POST['fuel']."','".$_POST['kwh']."','".$_POST['startreading']."','".$_POST['stopreading']."')");
			$totalmi+=$arr[$key];
		}
	...
	header("location:genset.php?".$_POST['olr_url']);
}
$total=0;
$header='<tr><th class="headcol" style="border-width:0px;">&nbsp;</th>';
...
<style>
	...
	#table123 thead, #table123 tbody, #table123 tr, #table123 td, #table123 th { display: block; }
	...
	#table123 tbody td, #table123 thead th {
	    width: 9%;
	    float: left;
	}
...
<div class="page-container">
	...
	<div class="page-content-wrapper">
		<div class="page-content">
			...
			<div class="modal fade bs-modal-lg" id="gensetdowntime" tabindex="-1" role="gensetdowntime" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						<form method="post" id="gensetform" action="genset.php?act=submitdowntime">
							 	...
							 	<div class="form-group" style="height:320px">	
									...
									<div class="col-md-9 margin-bottom-10">
										<div class="input-group date form_datetime">
											<input type="text" size="16" name="startd" id="startd" readonly class="form-control" onchange="checkinput();" required="required">
									...
									<div class="row">
										<label class="control-label col-md-3">Fuel Consumption (liters):</label>
								 		<div class="col-md-9 margin-bottom-10">
									...
									<div class="row">
								 		<label class="control-label col-md-3">KWH Delivered:</label>
								 		<div class="col-md-9 margin-bottom-10">
									...
									<div class="row">
								 		<label class="control-label col-md-3">Start:</label>
								 		<div class="col-md-9 margin-bottom-10">
								 			<input type="number" class="form-control" value="0.00" step="0.01" onchange="compute_hours()" name="startreading" id="startreading">
								 		</div>
									</div>
									...
									<div class="row">
								 		...
								 		<div class="col-md-9 margin-bottom-10">
								 			<input type="number" class="form-control" value="0.00" step="0.01" onchange="compute_hours()" name="stopreading" id="stopreading">
								 		</div>
									</div>
									<div class="row">
			...
			<div class="modal fade bs-modal-lg" id="munit" tabindex="-1" role="munit" aria-hidden="true">
				<div class="modal-dialog">
					 <form method="post" action="genset.php?act=newunit">
					<div class="modal-content">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
			...
			<!-- BEGIN PAGE HEADER-->
			<form method="get" act="genset.php">
			<div class="row">
				<div class="col-md-12">
					...
					<ul class="page-breadcrumb breadcrumb">
						...
						<li>			
							...
							<select class="form-control input-sm" name="s_location" id="s_location" onchange="changefilters();">
				 				<?php
				 					...
				 					$uq=sqlsrv_query($conn,"select distinct location from unit where  id in (9,10,11,16,17,18,24,25,26) order by location");
				 					while($u=sqlsrv_fetch_array($uq)){
				 						$select='';
				 						if(isset($_GET['s_location'])){
						...
						<li>							
							<select class="form-control input-sm" name="s_type" id="s_type"onchange="changefilters();">
				 				<?php
				 					...
				 					$uq=sqlsrv_query($conn,"select distinct type from unit where  id in (9,10,11,16,17,18,24,25,26) order by type");
				 					while($u=sqlsrv_fetch_array($uq)){
				 						$select='';
				 						if(isset($_GET['s_type'])){
						...
						<li id="unitlist">
							<select class="form-control input-sm" name="s_name" id="s_name">
				 				<?php
				 					...
				 					$uq=sqlsrv_query($conn,"select distinct name from unit where  id in (9,10,11,16,17,18,24,25,26) order by name");
				 					while($u=sqlsrv_fetch_array($uq)){
				 						$select='';
				 						if(isset($_GET['s_name'])){
						...
						<li>
							...
							<a href="genset.php" class="btn purple btn-sm" style="color:white;">Reset</a>
						</li>				
			...
			<div class="row margin-bottom-10">
				<div class="col-md-12">
					<div class=" pull-right">
						...
						<a href="rpt_genset_utilization.php" target="blank" class="btn blue-madison">Daily Utilization Report</a>
						<a href="#" onclick="exportToExcel('#tabled');" class="btn purple">Download Raw Data</a>
					</div>					
				</div>
			</div>
			<div class="clearfix">
					...
					<div class="portlet solid bordered grey-cararra">
						...
						<div class="portlet-body" style="overflow: auto;">
							<?php
								$data='';
								$ucntr=0;
								$catq=sqlsrv_query($conn,"select distinct type from unit where id in (9,10,11,16,17,18,24,25,26) ".$cond." ");
								while($cat=sqlsrv_fetch_array($catq)){
									$data.='<tr><th align="center" class="headcol text-center"><font style="font-weight:bold;color:blue;font-size:16px;">'.$cat['type'].'</font></th><td colspan="'.$total.'">&nbsp;</td></tr>';
									$locq=sqlsrv_query($conn,"select distinct location from unit where  id in (9,10,11,16,17,18,24,25,26) and type='".$cat['type']."' ".$cond."");
									while($loc=sqlsrv_fetch_array($locq)){
										$data.='<tr><th class="headcol text-center">'.$loc['location'].'</th><td colspan="'.$total.'">&nbsp;</td></tr>';
										$uq=sqlsrv_query($conn,"select * from unit where id in (9,10,11,16,17,18,24,25,26) and type='".$cat['type']."' and location='".$loc['location']."' ".$cond."");
										while($u=sqlsrv_fetch_array($uq)){
											$ucntr++;
											$data.='<tr>
														
														<th align="center" class="headcol text-center"><a style="display:none;" href="#" onclick=\'window.open("unit_edit.php?id='.$u['id'].'","displayWindow","toolbar=no,scrollbars=yes,width=910,height=600"); return false;\';>'.$ucntr.'</a>&nbsp;&nbsp;&nbsp;
														<a href="#" style="font: normal 13px/150% Arial, Helvetica, sans-serif;color:black;" onclick=\'window.open("unit_edit.php?id='.$u['id'].'","displayWindow","toolbar=no,scrollbars=yes,width=910,height=600"); return false;\';>'.$u['name'].'</a>
														</th>';
														foreach(interval($_GET['startDate'],$_GET['endDate'],$_GET['datetype']) as $x){														
															if($_GET['datetype']=='Weekly'){
																$lastdate=date('Y-m-d', strtotime("+6 days",strtotime($x)));
																$r=sqlsrv_fetch_array(sqlsrv_query($conn,"select sum(mins) as tmin from genset_utilizationflatdata where date>='".$x."' and date<='".$lastdate."' and unitId='".$u['id']."'"));
																$allremarks='';
																$remq=sqlsrv_query($conn,"select distinct remarks from genset_utilizationflatdata where date>='".$x."' and date<='".$lastdate."' and unitId='".$u['id']."'");
															...
															elseif($_GET['datetype']=='Daily'){
																...
																$r=sqlsrv_fetch_array(sqlsrv_query($conn,"select sum(mins) as tmin from genset_utilizationflatdata where date='".$x."' and unitId='".$u['id']."'"));																
																$allremarks='';
																$remq=sqlsrv_query($conn,"select distinct remarks from genset_utilizationflatdata where date>='".$x."' and unitId='".$u['id']."'");
																while($rem=sqlsrv_fetch_array($remq)){
																	$allremarks.=$rem['remarks'].'';
																}
															}
															...
															if($r['tmin']>0){																
																$hours = $r['tmin']/60;
																$mins=number_format($hours,2).' hrs';
																$datamins=' class="tdtab" data="'.$hours.'"';
																$bgcolor="background-color:green;color:white;";
															}
															else{
																$bgcolor='background-color:green;color:black;';
																$mins='0';
																$datamins=' class="tdtab" data="0"';
															}	
						...
						<div class="portlet-body">
							<div class="scroller" style="height: 400px;" data-always-visible="1" data-rail-visible="0">
								<ul class="feeds">
									...
									<?php 
										...
										$ldata='';
										$sec_data='';
										$lt=sqlsrv_fetch_array(sqlsrv_query($conn,"select count(d.id) as totald from genset_utilization d where d.unitId in (9,10,11,16,17,18,24,25,26)"));
										//$seq=$lt['totald'];
										$lq=sqlsrv_query($conn,"select CONVERT(VARCHAR(19),d.dateStart) as ds,CONVERT(VARCHAR(19),d.dateEnd) as de,d.*,u.name as uni from genset_utilization d left join unit u on u.id=d.unitId where d.unitId in (9,10,11,16,17,18,24,25,26) order by d.id desc");
										while($l=sqlsrv_fetch_array($lq)){
											...
											echo '
												<tr>
													...
													<td>'.$l['fuel'].'</td>
													<td>'.$l['kwh'].'</td>
													<td align="right">'.$l['runStart'].'</td>
													<td align="right">'.$l['runStop'].'</td>
													<td align="right">'.($l['runStop'] - $l['runStart']).'</td>
													<td>'.$l['remarks'].'</td>
													<td style="width:100px;"><a href="#" class="btn purple btn-sm" onclick=\'window.open("downtime_edit_genset.php?id='.$l['id'].'","displayWindow","toolbar=no,scrollbars=yes,width=910,height=600"); return false;\';><i class="fa fa-edit"></i></a>
							...
							<div class="scroller-footer">
								<div class="btn-arrow-link pull-right">
									<a href="genset_utilization_list.php" target="_blank">See All Records</a>
	...
	<table id="tabled" style="display:none;">
		<tr>
			<th>#</th>
			<th>Unit</th>
			<th>Start</th>
    ...
    $("#slider_2").trigger("change");
     ...
     $('.deletedl').click(function(){    	
       ...
       bootbox.confirm("Are you sure you want to delete this record?", function(result) {
           if(result){
             window.location = "genset.php?delete=delete&id="+x;
         }
     }); 
   });
});
...
<script>
	...
	function refresh_all(){
		...
		window.location.href = "genset.php?startDate="+start+"&endDate="+end+"&datetype="+datetype+"&s_location="+s_location+"&s_type="+s_type+"&s_name="+s_name;
		//alert(datetype);
	}
	function checkinput(){
		var StartDate= $("#startd").val();
			...
			if ($("#unit").val() != "" && $("#startd").val() != "" && $("#endd").val() != "" ){
			 	 $.ajax({
				  method: "POST",
				  url: "ajax.php?act=checkinputgenset",
