...
if(isset($_GET['act'])){
	if($_GET['act']=='editdowntime'){
		$delete=sqlsrv_query($conn,"delete from genset_utilizationflatdata where downtimeId='".$id."'");
		
		$upd=sqlsrv_query($conn,"update genset_utilization set unitId = '".$_POST['unit']."', 
		...
		foreach ($arrd as $key => $value){
			...
			$ins=sqlsrv_query($conn,"insert into genset_utilizationflatdata (date,mins,unitId,downtimeId,remarks,fuel,kwh)
				VALUES('".$value."','".$arr[$key]."','".$_POST['unit']."','".$lastins."','".$_POST['remarks']."','".$_POST['fuel']."','".$_POST['kwh']."')");
			$totalmi+=$arr[$key];
		}
	}
	
}
$r=sqlsrv_fetch_array(sqlsrv_query($conn,"select CONVERT(VARCHAR(16),dateStart,120) as ds,CONVERT(VARCHAR(16),dateEnd,120) as de,* from genset_utilization where id='".$id."'"));
?>
<html lang="en">
<!--<![endif]-->
<!-- BEGIN HEAD -->
...
<form method="post" action="downtime_edit_genset.php?act=editdowntime&id=<?php echo $id;?>">
				...
				<div class="row">
										<label class="control-label col-md-3">Fuel Consumption (liters):</label>
								 		<div class="col-md-9 margin-bottom-10">
								 			<input type="number" class="form-control" step="0.01" value="<?php echo $r['fuel'];?>" name="fuel" id="fuel">
								 		</div>
							 		</div>
									...
									<div class="row">
								 		...
								 		<div class="col-md-9 margin-bottom-10">
								 			<input type="number" class="form-control" value="<?php echo $r['kwh'];?>" step="0.01" name="kwh" id="kwh">
								 		</div>
									</div>								
